...
bin/sweep-pr status [123]                        # Review-sweep state of a PR, current branch's by default (see "Review sweep")
...
`Csv::ImportTransactionsJob` does the same for parsed `Csv::Transaction` rows. A CSV row carries an optional `remote_id` copied from the mapping's `id_column` (the export's stable per-transaction id); the job checks that identity against prior imports into the same ledger account first, then `Transaction::Reconcile`, then the description-based fallback for rows already merged into a transfer (issue 253). Editing an old import's mapping and re-parsing backfills `remote_id` on its existing rows without disturbing their ledger links.
...
Passkeys are usernameless WebAuthn credentials (`webauthn` gem) stored in `Passkey` (`external_id`, `public_key`, `sign_count`, `nickname`) and enrolled, renamed, and removed from the Settings page; each user carries an opaque `webauthn_id` handle. `PasskeysController` runs the registration ceremony and `Sessions::PasskeysController` the login ceremony; both are JSON endpoints driven by `app/javascript/controllers/passkey_controller.js`, which uses the browser's own `parseCreationOptionsFromJSON` / `toJSON` helpers. Challenges live in `webauthn_challenges` (`WebauthnChallenge.issue` / `.consume`), never in the cookie session, so consuming one is an atomic row delete and a registration challenge is bound to the user it was issued to; the login ceremony locks the `Passkey` row while it verifies and advances `sign_count`. An unnamed passkey is named by `Passkey::DefaultNickname` from the authenticator AAGUID (known providers) or the browser and platform in the user agent. Enrolment is gated by `RecentAuthentication`: the session must be under ten minutes old or the password re-confirmed through `ReauthenticationsController`. The relying party (`WebauthnRelyingParty` concern) derives its origin from the request in development and test; production pins it with `WEBAUTHN_ALLOWED_ORIGINS` (and optionally `WEBAUTHN_RP_ID`) and hides passkeys until that is set. Tests drive the ceremonies with `WebAuthn::FakeClient` through `test/test_helpers/passkey_test_helper.rb`.
...
- **Merging:** `gh pr merge --squash --delete-branch`, with the default subject so GitHub appends `(#PR)` itself. Passing `--subject` skips that suffix. Replace the generated commit list in the squash body with a summary of what landed (`--body-file`, or paste over it in the web UI); the round-by-round history stays in the PR. The body is an ordinary commit message: hard-wrap at 72 columns, open with a short paragraph on why, then bullets for what changed (1ff295a is the model). `git log` in a terminal is the audience, so a single long line is wrong even though GitHub renders it fine. If the title needs cleaning, `gh pr edit --title` first, then merge with the default. `--delete-branch` also removes the local branch and fast-forwards local `main`.
...
Bot reviews are nondeterministic, so a PR is reviewed in bounded rounds, not until both bots fall silent. The step-by-step procedure is the `sweep-pr` skill: the canonical copy is `.agents/skills/sweep-pr/SKILL.md`, and `.claude/skills/sweep-pr/SKILL.md` (Claude Code, invoked as `/sweep-pr N`) and `.github/skills/sweep-pr/SKILL.md` (Copilot) are links to it — edit only the canonical file. Claude Code reads the link at session start; `/reload-skills` may not notice edits made through it, so start a fresh session after changing the skill. `bin/sweep-pr` is the script behind it and works for any agent.
...
5. Stop when every finding is in a terminal state, CI is green on the head, and the delta is reviewed. Two full rounds is the limit; findings that surface later get human adjudication in the ledger, not another round. A bot's "clean" is evidence, not certification; Copilot is the primary defect finder for Rails changes and Codex is independent coverage, and they need not agree. For stacked PRs, sweep the parent to a met stop state before starting the child's sweep; once every PR in the stack is met, the stack can be merged as one unit from GitHub's stack view, or the parent merged on its own first — in that case the retargeting rule under "Stacked PRs" above applies.
