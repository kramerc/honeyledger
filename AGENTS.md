...
- **Merging:** `gh pr merge --squash --delete-branch`, with the default subject so GitHub appends `(#PR)` itself. Passing `--subject` skips that suffix. Replace the generated commit list in the squash body with a summary of what landed (`--body-file`, or paste over it in the web UI); the round-by-round history stays in the PR. If the title needs cleaning, `gh pr edit --title` first, then merge with the default. `--delete-branch` also removes the local branch and fast-forwards local `main`.
