...
This skill takes one argument, the pull request number, written as `N` below. It is the same procedure for every agent (Claude Code, Codex, Copilot): the canonical copy lives at `.claude/skills/sweep-pr/SKILL.md` (Claude Code only loads a real file there) and the copies under `.agents/skills/` and `.github/skills/` are links to it.
...
When green and the PR is still a draft: `gh pr ready N`. That alone triggers Copilot and Codex, so it *is* round one — do not also request.

## 3. One concurrent request

```
bin/sweep-pr request N
```

It skips any bot that has already reviewed the head or has a request pending, and refuses to start a third round without `--force`. Then poll `bin/sweep-pr status N` about every two minutes for up to fifteen minutes until both bots are done for the head. If `status` reports the Codex request as dropped, run `request` once more. Never hand-post `@codex review` or the Copilot request.

## 4. Collect

```
bin/sweep-pr findings N > tmp/sweep-N.json
```
...
Decide each one, in a ledger file (`tmp/sweep-N-ledger.json`, the shape `bin/sweep-pr ledger N` prints — start from that output so no existing entry is dropped):
...
```
bin/sweep-pr ledger N --write tmp/sweep-N-ledger.json
```

Findings you leave out are added as `open`; entries that do not justify their status, or that drop an entry the ledger already holds, are refused.
...
For each **inline** finding whose `replies` has no maintainer entry, reply in its thread once, one-way, following the `AGENTS.md` GitHub Interaction rules: fixed → `Fixed in <sha>. <what changed, which test covers it>`; rejected, deferred, or duplicate → open with the attribution line ("Written by <your agent name>, the agent that authored this PR."), give the cost-versus-cost, and offer to implement the alternative.

```
gh api -X POST repos/{owner}/{repo}/pulls/N/comments/<comment_id>/replies -F body=@tmp/reply.md
```
