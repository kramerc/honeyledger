...
- **Commit subjects** describe the change and never carry `(#NN)` issue references (every force-push would re-fire a cross-reference event on the issue). Put `Closes #NN` once in the PR body; PR titles may reference issues.
