...
class Simplefin::ImportTransactionsJobTest < ActiveJob::TestCase
  ...
  test "bumps the ledger transaction's synced_at when re-encountering a secondary SimpleFIN source" do
    # Set up: bank account linked to both Lunch Flow (canonical) and SimpleFIN.
    sf_account, bank_account = create_linked_simplefin_account(remote_id: "acc_secondary_sync", name: "Secondary Sync Bank")
    lunchflow_account = Lunchflow::Account.create!(
      connection: lunchflow_connections(:one),
      remote_id: 9990, name: "Secondary Sync Bank",
      institution_name: "Test Bank", provider: "finicity",
    ...
    ledger.reload
    assert_operator ledger.synced_at, :>, secondary_synced_at,
      "ledger.synced_at must advance so the outer query stops re-matching this secondary source"
  end

  test "rolls back the iteration when TransactionSource::Attach hits a concurrent-job race" do
    sf_account, _ = create_linked_simplefin_account(remote_id: "acc_race", name: "Race Bank")
    Simplefin::Transaction.create!(
      account: sf_account, remote_id: "sf_race", amount: "-10.00",
      description: "Coffee", posted: 1.day.ago, transacted_at: 1.day.ago, pending: false
    )

    assert_no_difference "Transaction.count" do
      TransactionSource::Attach.stub :call, ->(*) { raise TransactionSource::Attach::MismatchedTransaction, "race" } do
        Simplefin::ImportTransactionsJob.perform_now(simplefin_account_id: sf_account.id)
      end
    end
  end

  test "skips the row when the Reconcile-match attach hits a concurrent-job race" do
    ...
    Simplefin::Transaction.create!(
      account: sf_account, remote_id: "sf_reconcile_race", amount: "-10.00",
      description: "Coffee", posted: 1.day.ago, transacted_at: 1.day.ago, pending: false
    )

    # Reconcile.call returns the manual-entry txn, then Attach raises (simulating
    # a parallel job that already wrote the source row). The job must skip.
