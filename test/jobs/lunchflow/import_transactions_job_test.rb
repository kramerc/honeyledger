...
class Lunchflow::ImportTransactionsJobTest < ActiveJob::TestCase
  ...
  test "bumps the ledger transaction's synced_at when re-encountering a secondary Lunch Flow source" do
    ...
    sf_account = Simplefin::Account.create!(
      connection: simplefin_connections(:one), remote_id: "acc_lf_secondary",
  ...
  test "rolls back the iteration when TransactionSource::Attach hits a concurrent-job race" do
    lf_account, _ = create_linked_lunchflow_account(remote_id: 9992, name: "Race Bank")
    Lunchflow::Transaction.create!(
      account: lf_account, remote_id: "lf_race", amount: "-10.00",
      currency: "USD", description: "Coffee", pending: false, date: 1.day.ago.to_date
    )
  ...
  test "skips the row when the Reconcile-match attach hits a concurrent-job race" do
    lf_account, bank_account = create_linked_lunchflow_account(remote_id: 9993, name: "Reconcile Race Bank")
    ...
    Lunchflow::Transaction.create!(
      account: lf_account, remote_id: "lf_reconcile_race", amount: "-10.00",
      currency: "USD", description: "Coffee", pending: false, date: 1.day.ago.to_date
    )
