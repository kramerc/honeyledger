...
class ApplicationHelperTest < ActionView::TestCase
  ...
  test "nav_link_to with :prefix emits data-nav-active-path" do
    result = nav_link_to("Accounts", "/accounts", active: :prefix)
    assert_match(/data-nav-active-path="\/accounts"/, result)
  end

  test "nav_link_to with String active emits data-nav-active-path of that string" do
    result = nav_link_to("Show", "/accounts/1/transactions", active: "/accounts/1")
    assert_match(/data-nav-active-path="\/accounts\/1"/, result)
  end

  test "nav_link_to with no active option does not emit data-nav-active-path" do
    result = nav_link_to("Home", "/")
    assert_no_match(/data-nav-active-path/, result)
  end
end
