class Simplefin::RefreshJob < ApplicationJob
  queue_as :default

  def perform(simplefin_connection_id = nil)
    connections = if simplefin_connection_id
      Simplefin::Connection.where(id: simplefin_connection_id)
    else
      Simplefin::Connection.where("refreshed_at IS NULL OR refreshed_at < ?", 1.day.ago)
    end
